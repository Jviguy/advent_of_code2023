...
class Day10Solution extends DaySolution {
  ...
  int breadthFirstFill(List<String> lines, (int, int) startingPos) {
    ...
    for ((int, int) adj in direcList[lines[startingPos.$1][startingPos.$2]]!) {
      ...
      if (newRow >= 0 &&
          ...
          newCol < lines.length) {
        // new valid position.
        if (checkValidStartingConnection(
            lines, (newRow, newCol), startingPos)) {
    ...
    while (q.isNotEmpty) {
      ...
      for ((int, int) diff in direcList[lines[pos.$1][pos.$2]]!) {
        ...
        if (checkValidStartingConnection(
            lines, (pos.$1 + diff.$1, pos.$2 + diff.$2), pos)) {
  ...
  bool checkValidStartingConnection(
      List<String> lines, (int, int) pipe, (int, int) startingPos) {
